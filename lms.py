...
def p(*args, **kwargs):
    ...
    for i in range(len(args)):
        length += wcswidth(args[i].replace('\r', ''))
    length += len(args) - 1
...
class Lms:
    ...
    def login(self):
        ...
        webdriver.ActionChains(self.driver)\
            ...
            .perform()
        try:
            is_login_complete = self.driver.current_url.find("login") > 0
    ...
    def learn(self):
        ...
        try:
            while True:
                ...
                self.print_progress(current_progress, played, length)
